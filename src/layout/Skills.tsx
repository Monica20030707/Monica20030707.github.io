...
const TechIcon = ({ tech, index, isVisible, delay = 0 }: { 
  ...
  <div 
    key={`${tech.name}-${index}`}
    className={`flex-shrink-0 mx-3 sm:mx-4 group cursor-pointer ${isVisible ? 'animate-fade-in' : 'opacity-0'}`}
    style={{
    ...
    >
      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        <div className="max-w-6xl mx-auto">
          ...
          >
            <h2 className={`text-3xl sm:text-4xl lg:text-5xl font-bold text-beige mb-4 font-montserrat ${isVisible ? 'animate-fade-in' : 'opacity-0'}`}
              style={{ animationDelay: '0.2s' }}
            >
              Skills & Technologies
            </h2>
            <p 
              className={`text-center text-beige/80 mb-6 sm:mb-8 text-sm sm:text-base font-montserrat ${isVisible ? 'animate-fade-in' : 'opacity-0'}`}
          ...
          <div className="space-y-12 sm:space-y-16">
            ...
            <div className="relative">
              <h3 
                className={`text-lg sm:text-xl font-semibold text-blush mb-6 sm:mb-8 text-center font-montserrat ${isVisible ? 'animate-slide-in-left' : 'opacity-0'}`}
              ...
              <div className="relative overflow-hidden py-2 sm:py-4">
                ...
                >
                  {duplicatedFrontend.map((tech, index) => (
                    <TechIcon key={`frontend-${index}`} tech={tech} index={index} isVisible={isVisible} delay={1 + index * 0.05} />
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="relative">
              <h3 
                className={`text-lg sm:text-xl font-semibold text-blush mb-6 sm:mb-8 text-center font-montserrat ${isVisible ? 'animate-slide-in-right' : 'opacity-0'}`}
              ...
              <div className="relative overflow-hidden py-2 sm:py-4">
                ...
                >
                  {duplicatedBackend.map((tech, index) => (
                    <TechIcon key={`backend-${index}`} tech={tech} index={index} isVisible={isVisible} delay={1.4 + index * 0.05} />
                  ))}
                </div>
              </div>
            </div>
              ...
              <div className="relative overflow-hidden py-2 sm:py-4">
                ...
                >
                  {duplicatedTools.map((tech, index) => (
                    <TechIcon key={`tools-${index}`} tech={tech} index={index} isVisible={isVisible} delay={1.8 + index * 0.05} />
                  ))}
                </div>
              </div>
            </div>
